...
st.sidebar.markdown("---")
st.sidebar.caption(
    "✅ Powered by Yahoo Finance — free, no API key needed, no daily limits!"
)
...
def get_stock_data(symbol):
    try:
        ...
        if roe is not None:
            roe = round(float(roe) * 100, 2)  # convert 0.35 → 35%
...
if st.button("🚀 Screen Stocks"):
    ...
    for i, symbol in enumerate(symbols):
        ...
        if pe_ok and roe_ok and vol_ok and ind_ok:
            ...
            reasons = []
            if not pe_ok:
                reasons.append(f"PE={round(pe,1) if pe else 'N/A'} (max {pe_max})")
            if not roe_ok:
                reasons.append(f"ROE={round(roe,1) if roe else 'N/A'}% (min {roe_min}%)")
            if not vol_ok:
                reasons.append(f"Vol too low")
            if not ind_ok:
